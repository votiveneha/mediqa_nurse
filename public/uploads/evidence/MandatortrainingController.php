...
class MandatortrainingController extends Controller{
    ...
    public function updateMandatoryTraining(Request $request){
        ...
        if (!empty($gettrainingdata) > 0) {
            $run = MandatoryTrainModel::where('user_id', $user_id)->update(["well_sel_data"=>$well_institution,"core_man_data"=>$core_man_institution,"other_tra_data"=>$other_tra_json,"other_edu_data"=>$other_edu_json,"declaration_status"=>$declare_information_man]);
        }else{
            $post = new MandatoryTrainModel();
            $post->user_id = $user_id;
            $post->well_sel_data   = $well_institution;
            $post->core_man_data   = $core_man_institution;
